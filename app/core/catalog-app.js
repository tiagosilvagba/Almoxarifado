Warning: truncated output (original token count: 74851)
...
async function handleWorkerMessage(event) {
  ...
  try {
    // A cópia dos dados já chegou à interface. Encerrar o worker libera
    // imediatamente os mapas, textos e históricos mantidos durante a leitura.
    // Sem isso, a aplicação conservava duas bases completas na memória.
    if (state.worker) {
      state.worker.terminate();
      state.worker = null;
    }
...
function getItemPurchaseCommitments(item) {
  ...
  for (const request of requests.values()) {
    ...
    if (!(quantity > 0)) contin…14851 tokens truncated…lierCode,
  ].join(" ")).includes(query);
}
...
function inventoryWorker() {
  ...
  self.onmessage = async (event) => {
    try {
      ...
      const [saldoText, comprasSources, replenishmentText, consumoText, baseUpdatedAt] = await Promise.all([
        ...
        discoverPurchaseParts(comprasApiUrl, comprasFallbackUrl),
      ...
      parseCsv(saldoText, (headers, row) => {
        ...
        const position = {
          ...
          responsibleAreas: resolveReplenishmentAreas(
            replenishmentByLocation,
            branchCode,
            localCode,
            resolvedBranchName,
      ...
      for (let sourceIndex = 0; sourceIndex < comprasSources.length; sourceIndex += 1) {
        ...
        progress(`Baixando parte ${sourceIndex + 1} de ${comprasSources.length}: ${comprasSource.name}…`, 55 + (sourceIndex / Math.max(1, comprasSources.length)) * 30);
        const comprasText = await fetchOptionalText(comprasSource.downloadUrl);
        if (comprasText === null) {
          progress(`Arquivo ignorado por indisponibilidade: ${comprasSource.name}…`);
          continue;
        }
  ...
  async function discoverPurchaseParts(apiUrl, fallbackUrl) {
    let partFiles = [];
    if (apiUrl) {
      try {
    ...
    if (partFiles.length) {
      return partFiles.map((file) => ({
        name: file.name,
        downloadUrl: new URL(file.download_url, apiUrl).href,
      }));
    }

    return fallbackUrl ? [{ name: "01 - Compras_Almox.csv", downloadUrl: fallbackUrl }] : [];
  }
  ...
  function parseReplenishmentCsv(text) {
    ...
    parseCsv(meaningfulText, (headers, row) => {
      ...
      const area = clean(get("Área")) || clean(get("Area"));
      if (!branchCode || !localCode || (!responsible && !area)) return;
      ...
      if (!values.some((value) => normalizeText(value.responsible) === normalizeText(responsible)
        && normalizeText(value.area) === normalizeText(area)
        && normalizeBranchIdentity(value.branchName) === normalizeBranchIdentity(branchName))) {
        values.push({ responsible, area, branchName });
      }
  ...
  function resolveReplenishmentResponsibles(assignments, branchCode, localCode, resolvedBranchName) {
    ...
    return values
      ...
      .filter((entry) => entry.responsible)
      .map((entry) => entry.responsible);
  }

  function resolveReplenishmentAreas(assignments, branchCode, localCode, resolvedBranchName) {
    const values = assignments.get(responsibleLocationKey(branchCode, localCode)) || [];
    const resolvedIdentity = normalizeBranchIdentity(resolvedBranchName);
