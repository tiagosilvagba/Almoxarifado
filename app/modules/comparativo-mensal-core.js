...
(() => {
  const REPO_CONTENTS_API = "data-manifest.json";
  const RESPONSIBILITY_FILE = "./02 - Responsaveis_Reposição.CSV?area=20260914-1";
  ...
  let responsibilityLoaded = false;
  ...
  function normalizeKeyPart(value) {
    ...
    if (/^0*\d+$/.test(text)) return String(Number(text));
    return normalize(text);
  }

  function positionKey(branch, local) {
    return `${normalizeKeyPart(branch)}::${normalizeKeyPart(local)}`;
  }
  ...
  async function loadResponsibilityAreas() {
    if (responsibilityLoaded) return;
    const parsed = parseCsv(await fetchText(RESPONSIBILITY_FILE, "no-store"));
    const branchIndex = findColumn(parsed.headers, ["FILIAL","CD FILIAL","Cd Filial"], ["filial"]);
    const localIndex = findColumn(parsed.headers, ["CD LOCAL","Cd Local","Local"], ["cd local"]);
    const areaIndex = findColumn(parsed.headers, ["Área","Area","ÁREA","AREA"], ["area"]);
    if (branchIndex < 0 || localIndex < 0 || areaIndex < 0) {
      responsibilityLoaded = true;
      return;
    }
    for (const row of parsed.rows) {
      ...
      const key = positionKey(branch, local);
      const values = areaByPosition.get(key) || new Set();
      values.add(area);
      areaByPosition.set(key, values);
    }
    responsibilityLoaded = true;
  }

  function areasForPosition(branch, local) {
    return [...(areaByPosition.get(positionKey(branch, local)) || [])];
  }
  ...
  function buildSnapshot(file, parsed) {
    ...
    const codeIndex = findColumn(headers,
      ["Item","ITEM","Código","Codigo","Código item","Codigo item","Cd Item","CD_ITEM","Material"],
      ["codigo item","cd item"]);
    const nameIndex = findColumn(headers,
      ["Nome Item","NM Item","Nm Item","NM_ITEM","Nome do Item","Descrição","Descricao","Descrição Item","Descricao Item","Descrição do Item","Descricao do Item","Nome Material","Descrição Material","Descricao Material"],
      ["nome item","nm item","descricao item","descricao material"]);
    ...
    if (codeIndex < 0) throw new Error(`O arquivo ${file.name} precisa conter a coluna “Item” ou um código de item equivalente.`);
    ...
    rows.forEach((row) => {
      ...
      const local = localIndex >= 0 ? String(row[localIndex] ?? "").trim() : "";
      records.push({
        code,
        ...
        areas:areasForPosition(branch, local),
      });
    });
    const snapshot = aggregateRecords(file, headers, records);
    snapshot.filterColumns = {
      branch:branchIndex >= 0,
      local:localIndex >= 0,
      usage:usageIndex >= 0,
      area:areaByPosition.size > 0,
      name:nameIndex >= 0,
      code:codeIndex >= 0,
    };
    return snapshot;
  ...
  async function fetchSnapshot(file) {
    ...
    const promise = (async () => buildSnapshot(file, parseCsv(await fetchText(`./${file.name}`))))();
  ...
  function uniqueValues(snapshots,key) {
    ...
    snapshots.filter(Boolean).forEach((snapshot) => snapshot.records.forEach((record) => {
      const current = record[key];
      if (Array.isArray(current)) current.forEach((value) => value && values.add(value));
      else if (current) values.add(current);
    }));
  ...
  function populatePageFilters(...snapshots) {
    ...
    fillFilterSelect("monthlyAreaFilter",uniqueValues(snapshots,"areas"),"Área");
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .month-compare-actions{display:flex;gap:8px;align-items:center}.month-compare-actions .button{min-height:44px}
      ...
      .month-page-filters__grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;align-items:end}.month-page-filters label,.month-compare-controls label{display:grid;gap:7px;font-weight:700}
      ...
      .month-page-filters__actions{display:flex;gap:8px;grid-column:1/-1;justify-content:flex-end}.month-page-filters__actions .button{min-height:44px}
      ...
      @media(max-width:1100px){.month-page-filters__grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
      @media(max-width:900px){.month-compare-controls{grid-template-columns:1fr}.month-compare-actions{width:100%}.month-compare-actions .button{flex:1}.month-page-filters__grid{grid-template-columns:1fr}.month-page-filters__actions{width:100%}.month-page-filters__actions .button{flex:1}.month-compare-metrics{grid-template-columns:repeat(2,minmax(0,1fr))}.month-considerations{grid-template-columns:1fr}.month-compare-toolbar{align-items:stretch;flex-direction:column}.month-compare-toolbar input{max-width:none}.month-compare-metric strong{font-size:20px}}
    `;
      ...
      <section id="page-comparativo-mensal" class="page-panel is-hidden" data-page-panel="comparativo-mensal" aria-labelledby="monthlyCompareTitle">
        ...
        <section class="month-page-filters" aria-label="Filtros exclusivos do comparativo mensal">
          ...
          <div class="month-page-filters__grid">
            ...
            <label>Área<select id="monthlyAreaFilter"><option value="all">Todos</option></select></label>
        ...
        <div id="monthlyCompareDetails" class="is-hidden">
          <div class="month-compare-toolbar"><input id="monthlySearch" type="search" placeholder="Buscar item ou nome do item"><select id="monthlyStatusFilter"><option value="all">Todas as alterações</option><option value="increase">Somente aumentos</option><option value="decrease">Somente reduções</option><option value="stable">Sem alteração</option></select></div>
          <div class="month-compare-table-wrap"><table class="month-compare-table"><thead><tr><th>Item</th><th>Nome Item</th><th>Saldo base</th><th>Saldo comparado</th><th>Variação saldo</th><th>Consumo base</th><th>Consumo comparado</th><th>Variação consumo</th><th>Situação</th></tr></thead><tbody id="monthlyCompareTbody"></tbody></table></div>
        </div>
      </section>`;
  }
  ...
  function updateFilterSummary() {
    ...
    if (filters.area !== "all") parts.push(`Área ${filters.area}`);
  ...
  function fillSelectors() {
    ...
    if (availableFiles.length >= 2) {
      ...
      document.getElementById("monthlyExportExcel").disabled = false;
      ...
      document.getElementById("monthlyExportExcel").disabled = true;
    }
  ...
  function renderConsiderations(rows) {
    ...
    node.innerHTML = `<article class="month-consideration month-consideration--up"><h3>Itens que aumentaram</h3><p>${nf0.format(up.length)} itens tiveram aumento no Saldo Real.</p><div class="month-consideration__total"><span>Aumento acumulado</span><strong>+${nf2.format(upTotal)}</strong></div><ul>${list(up,"+")}</ul></article><article class="month-consideration month-consideration--down"><h3>Itens que reduziram</h3><p>${nf0.format(down.length)} itens tiveram redução no Saldo Real.</p><div class="month-consideration__total"><span>Redução acumulada</span><strong>−${nf2.format(downTotal)}</strong></div><ul>${list(down,"-")}</ul></article>`;
  }
  ...
  function statusLabel(status) {
    return status === "increase" ? "Aumentou" : status === "decrease" ? "Reduziu" : "Estável";
  }

  function xmlCell(value, type = "String", style = "") {
    const styleAttr = style ? ` ss:StyleID="${style}"` : "";
    const safe = type === "Number" ? (Number.isFinite(Number(value)) ? Number(value) : 0) : escapeXml(value);
    return `<Cell${styleAttr}><Data ss:Type="${type}">${safe}</Data></Cell>`;
  }

  function exportComparisonExcel() {
    if (!lastBaseSnapshot || !lastCurrentSnapshot || !currentRows.length) {
      setMessage("Faça uma comparação entre dois meses antes de baixar o Excel.", "warning");
      return;
    }
    const base = applySnapshotFilters(lastBaseSnapshot);
    const current = applySnapshotFilters(lastCurrentSnapshot);
    const rows = compareSnapshots(base,current);
    const filters = selectedFilters();
    const filterText = [
      filters.branch !== "all" ? `Filial: ${filters.branch}` : "",
      filters.local !== "all" ? `Local: ${filters.local}` : "",
      filters.usage !== "all" ? `Nome utilização: ${filters.usage}` : "",
      filters.area !== "all" ? `Área: ${filters.area}` : "",
    ].filter(Boolean).join(" | ") || "Sem filtros específicos";
    const increased = rows.filter((row) => row.status === "increase").length;
    const decreased = rows.filter((row) => row.status === "decrease").length;

    const summaryRows = [
      ["Mês base", base.file.label],
      ["Mês comparado", current.file.label],
      ["Filtros", filterText],
      ["Saldo base", base.totalBalance],
      ["Saldo comparado", current.totalBalance],
      ["Variação saldo", current.totalBalance - base.totalBalance],
      ["Consumo base", base.totalConsumption],
      ["Consumo comparado", current.totalConsumption],
      ["Variação consumo", current.totalConsumption - base.totalConsumption],
      ["Itens que aumentaram", increased],
      ["Itens que reduziram", decreased],
      ["Itens comparados", rows.length],
    ];

    const summaryXml = summaryRows.map(([label,value]) =>
      `<Row>${xmlCell(label,"String","Header")}${xmlCell(value, typeof value === "number" ? "Number" : "String")}</Row>`).join("");
    const header = ["Item","Nome Item","Saldo base","Saldo comparado","Variação saldo","Variação %","Consumo base","Consumo comparado","Variação consumo","Situação"];
    const detailXml = `<Row>${header.map((value) => xmlCell(value,"String","Header")).join("")}</Row>` + rows.map((row) =>
      `<Row>${[
        xmlCell(row.code),
        xmlCell(row.name),
        xmlCell(row.baseBalance,"Number"),
        xmlCell(row.currentBalance,"Number"),
        xmlCell(row.deltaBalance,"Number"),
        xmlCell(row.pctBalance == null ? "" : row.pctBalance, row.pctBalance == null ? "String" : "Number"),
        xmlCell(row.baseConsumption,"Number"),
        xmlCell(row.currentConsumption,"Number"),
        xmlCell(row.deltaConsumption,"Number"),
    ...
    const workbook = `<?xml version="1.0"?>
      ...
      <Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
        ...
        <Worksheet ss:Name="Resumo"><Table>${summaryXml}</Table></Worksheet>
        <Worksheet ss:Name="Comparativo"><Table>${detailXml}</Table></Worksheet>
      </Workbook>`;
    const blob = new Blob(["\uFEFF",workbook], { type:"application/vnd.ms-excel;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `Comparativo_Estoque_${base.file.monthKey}_${base.file.year}_x_${current.file.monthKey}_${current.file.year}.xls`;
  ...
  async function runComparison() {
    ...
    try {
      ...
      renderTable();
      updateFilterSummary();
      setMessage(`Comparação concluída: ${baseFile.label} → ${currentFile.label}. Item e Nome Item foram lidos da própria base mensal.`);
  ...
  function clearPageFilters() {
    ["monthlyBranchFilter","monthlyLocalFilter","monthlyUsageFilter","monthlyAreaFilter"].forEach((id) => {
      const select = document.getElementById(id);
  ...
  function bindUi() {
    ...
    document.getElementById("monthlyExportExcel")?.addEventListener("click",exportComparisonExcel);
